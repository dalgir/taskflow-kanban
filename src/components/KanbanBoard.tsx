...
import { Plus, Circle, Loader2, Timer, CheckCircle2, Award, AlertCircle, UserX, Layout, CheckCircle } from 'lucide-react';
...
function KanbanColumn({ column, tasks, onTaskClick, onAddTask, colorIndex = 0 }: KanbanColumnProps) {
  const { teamMembers, getAbsencesByDate } = useApp();
  const { setNodeRef, isOver } = useDroppable({ id: column.id });
  
  const member = column.memberId ? teamMembers.find(m => m.id === column.memberId) : null;
  
  // Verificar se o membro está ausente hoje
  const today = new Date();
  const memberAbsences = member ? getAbsencesByDate(today).filter(a => a.memberId === member.id) : [];
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "w-80 flex-shrink-0 bg-white rounded-xl shadow-sm border p-4 flex flex-col h-fit",
        column.type === 'completed' ? 'border-green-200 bg-green-50/30' : 
        isAbsent ? 'border-red-300 bg-red-50/30' : 'border-gray-200',
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4 flex-shrink-0">
        {column.type === 'backlog' ? (
          ...
          <div className={`relative ${isAbsent ? 'opacity-60' : ''}`}>
            {member.avatarUrl ? (
              <img 
                src={member.avatarUrl} 
                alt={member.name}
                className={`w-10 h-10 rounded-lg object-cover ${isAbsent ? 'grayscale' : ''}`}
              />
            ) : (
              <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${colors[colorIndex % colors.length]} flex items-center justify-center`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {member && isAbsent && absenceInfo && (
            <p className="text-xs text-red-600 font-medium truncate">{absenceInfo.title}</p>
          )}
        </div>
        
        <span className={cn(
          "text-xs font-medium px-2 py-1 rounded-full flex-shrink-0",
          column.type === 'backlog' ? 'bg-blue-100 text-blue-700' :
          column.type === 'completed' ? 'bg-green-100 text-green-700' :
          isAbsent ? 'bg-red-100 text-red-700' : 'bg-purple-100 text-purple-700'
        )}>
          {tasks.length}
        </span>
      </div>
      ...
      {column.type !== 'completed' && (
        <button
          ...
          className={cn(
            "mt-3 flex items-center justify-center gap-2 p-3 border-2 border-dashed rounded-xl transition-all flex-shrink-0",
            column.type === 'backlog' 
...
export function KanbanBoard() {
  ...
  return (
    <div>
      ...
      <div className="pb-4">
        ...
        <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-sm font-semibold text-gray-700">📋 Legenda de Status:</span>
          </div>
      ...
      {/* Kanban Board - Colunas lado a lado com scroll horizontal fixo */}
      <div className="overflow-x-auto bg-gray-50 rounded-xl border border-gray-200 p-4 sticky top-[73px]" style={{ maxHeight: 'calc(100vh - 240px)' }}>
        <DndContext
          sensors={sensors}
